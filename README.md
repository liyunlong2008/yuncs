...
### 2026-09-06 追加：杠杆与保证金敏感性（ma_macd vs rsi_revert，compounding）

| 杠杆(20U,固定5U保证金) | ma_macd 3年 | ma_macd 近1年 | rsi_revert 3年 | rsi_revert 近1年 |
|---|---|---|---|---|
| 5x | **0.998x** | **0.997x** | 0.015x | 0.015x |
| 10x | 0.971x | 0.989x | 0.007x | 0.008x |
| 20x | 0.932x | 0.972x | 0.003x | 0.003x |
| 50x | 0.705x（强平7） | 0.927x | 0.001x | 0.001x |

- **高杠杆对两策略都是单调变差，新策略"适合高杠杆"的直觉不成立**：136+小目标+锁利的结构把每笔
  收益压到手续费边缘，高杠杆只放大费用与尾部亏损（ma_macd 50x 出现强平、rsi 加速归零）。
- 提高保证金占比（5x 下 25%→50%，交易数翻倍）同样单调变差（3年 0.995→0.968x）——
  **信号层边际略负，资金层无解**；25%~5U 因价格高企时的"最小单量死区"反而减少了亏损暴露。
