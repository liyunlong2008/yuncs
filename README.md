...
**翻倍目标 + 回撤出局，不限时**：20U 起步，权益翻到目标倍数即挑战成功，从峰值回撤超限即平仓停止。
...
- 玩法适配回测：逐 K 线回放 + 资金费结算 + 强平检查 + 挑战规则，输出挑战视角报告（成功率/达标用时/回撤命中/强平次数）
- FastAPI + 单页 Web 看板：实时权益曲线、挑战进度、持仓（USDT 口径：可买/成本/预估成本价）、成交明细
- 配置全 TOML（无 env 文件），密钥独立 `secrets.toml`（gitignore）
...
app/
  ...
  challenge.py    挑战引擎（翻倍目标/回撤出局/可选时长）
